...
- **`noindex: true` marks text that isn’t Mandy’s.** The five D&D pages carry it;
  they are copy-pasted from Tiger Kong’s page and kept only so they surface in the
  blog’s own search, while `_includes/head.html:60` emits
  `<meta name="robots" content="noindex">` to keep them out of external search
  engines. Never sample them for prose or typography conventions:
  `grep -rL "^noindex:" blog/ --include="*.md"` gets the pages she wrote.
...
- **Permalinks:** `blog/<segments>/<slug>`, with no leading slash, no trailing slash, no
...
- **Typography is deliberate.** Curly quotes in prose (`’` U+2019, `“ ”`
  U+201C/D), straight quotes in code. Dashes are wrapped in **hair spaces**
  (U+200A), not normal spaces: `a — b` and `1 – 2 tbsp`. **Don’t over-use em
  dashes**; a colon, semicolon, period, or comma is usually better. Also
  `&nbsp;` to bind pairs, spaces inside parens `( like this )`, and `&#58;` for a
  literal colon inside a YAML scalar.
  `blog/design/type/copy-pasta.md` holds copy-pasteable canonical forms; its
  dash fences already include the hair spaces.
...
The JSON-LD on the page is unrelated, a generic `BlogPosting` from `jekyll-seo-tag`.
...
- Local plugins in `_plugins/` **do not run on GitHub Pages'** built-in Jekyll. This
  ...
  runs `bundle exec jekyll build` and pushes `_site` to the `gh-pages` branch, and that
